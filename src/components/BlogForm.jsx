...
const BlogForm = ({ initialData, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      if (dataToSubmit.imageurl) {
        dataToSubmit.imageurl = dataToSubmit.imageurl
          .replace(/\\"$/, "")
          .trim();
      }
  ...
  return (
    <div
      className={`max-w-2xl mx-auto p-8 rounded-lg shadow-xl shadow-yellow-500/50 border ${
        theme === "light"
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <input
            ...
            className={`mt-1 block w-full px-4 py-2 border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 ${
              theme === "light"
                ? "border-gray-300"
                : "border-gray-600 bg-gray-700 text-gray-100"
            } ${errors.bloggername ? "border-red-500" : ""}`}
          />
          {errors.bloggername && (
            <p className="mt-1 text-sm text-red-500">{errors.bloggername}</p>
          )}
        </div>
        <div>
          <label
        ...
        <div className="text-center">
          <button
            type="submit"
            className="py-3 px-6 bg-yellow-600 text-white font-bold rounded-md hover:bg-yellow-700 transition-colors"
          >
